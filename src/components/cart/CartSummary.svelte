<script lang="ts">
  ...
  const shippingProgress = $derived(
    freeShippingThreshold > 0 ? Math.min((subtotal / freeShippingThreshold) * 100, 100) : 0
  );
  const remainingForFreeShipping = $derived(Math.max(freeShippingThreshold - subtotal, 0));
...
<div class="cart-summary">
  ...
  <div class="summary-row summary-row-total">
    ...
    <span class="summary-value">{formatPrice(empty ? 0 : total + (hasFreeShipping ? 0 : shippingPrice))}</span>
  </div>
  ...
  <div class="shipping-info">
    ...
    <div class="shipping-text">
      ...
      {:else if freeShippingThreshold > 0 && !empty}
        <span class="shipping-progress-text">
          Mai ai nevoie de {formatPrice(remainingForFreeShipping)} pentru livrare gratuită
        </span>
